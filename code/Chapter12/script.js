...
d3Edge.dataManager = function module() {
  ...
  //Create a getDelays method to filter and aggreate the delay data by stop.
...
var genevaRadial = d3Edge.radialHistogram().colorRange(['lightblue', 'darkblue'])
  .innerRadius(5)
  .outerRadius(200)
  .offset(15)
  .radialRange([100,200]);

var sanFranRadial = d3Edge.radialHistogram().colorRange(['lightblue', 'darkblue'])
  .innerRadius(5)
  .outerRadius(200)
  .offset(15)
  .radialRange([100,200]);
...
var genevaHist = d3.select('#geneva_hist')
  .append('svg')
  .attr('width', width)
  .attr('height', height)
  .append('g')
...
var sanFranHist = d3.select('#sanFran_hist')
  .append('svg')
  .attr('width', width)
  .attr('height', height)
  .append('g')
...
//On map brushing, filter the stop metric data, pass filtered data into radial chart.
genevaMap.on('brushing', function (brush) {
  //Get the locations inside the brush.
  var filteredLocations = genevaDataManager.filterLocation(brush.extent()),
      //Get the locations inside the brush.
      delaysByHourAndLocation = genevaDataManager.getDelays(filteredLocations);

  //Pass in our filtered delays to the radial histogram.
  genevaRadial.group(delaysByHourAndLocation[1]).dimension(delaysByHourAndLocation[0]);
  //Update the radial chart with the new data.
  genevaHist.call(genevaRadial);
...
sanFranciscoMap.on('brushing', function (brush) {
  ...
  //Pass in our filtered delays to the radial histogram.
  sanFranRadial.group(delaysByHourAndLocation[1]).dimension(delaysByHourAndLocation[0]);
  //Update the radial chart with the new data.
  sanFranHist.call(sanFranRadial);
});
